...
router.post(
    ...
    (req, res) => {
        ...
        const query = "INSERT INTO actividades (nombre, descripcion, fecha, usuario_id) VALUES (?, ?, ?, ?)";
        db.query(query, [nombre, descripcion, fecha, usuario_id], (err, results) => {
            if (err) {
                console.error("Error registrando actividad:", err.message);
                return res.status(500).json({ error: "Error al registrar actividad" });
            }
            res.json({ message: "Actividad registrada con éxito", id: results.insertId });
        });
    }
);
